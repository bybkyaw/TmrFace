...
def calculate_heikin_ashi(df):
    ...
    ha_df['HA_Open'] = 0.0
    ha_df.at[df.index[0], 'HA_Open'] = (df.at[df.index[0], 'Open'] + df.at[
        df.index[0], 'Close']) / 2  # First row initial calculation

    # Sequentially calculate HA_Open for the rest of the rows
    for i in range(1, len(df)):
        ha_df.at[df.index[i], 'HA_Open'] = (ha_df.at[df.index[i - 1], 'HA_Open'] + ha_df.at[
